...
# url="/home/kolibri/Downloads/tinymce_5.1.0_dev/tinymce/js/tinymce/tinymce.js"
# url="/home/kolibri/Downloads/php-tiny_mce_wiris-7.17.0.1426/tiny_mce_wiris/integration/showimage.php"
url1="http://www.magogenie.com"
url2="https://www.magogenie.com"
# DESCRIPTION = "v0.1"
...
invalid_question_list = ['221832','131492','112502','47892','47930','47931','182660','182661','127207','127208','126544','183929','113276']
# invalid_question_list = ['183929','113276']
# CBSE-8,7,5,5,5,5,4,4 BalBharati-5,5,5
#cbse-4 '183929','113276'
arrlevels = []
# This method takes question id and process it
QUESTION_URL = "https://www.magogenie.com/v1/question/%s/39e84462-60f6-49a6-a540-b885e7802d07"




def question_list(question_ids):
    """ question_list: constructs the list of questions
        Args:
            question_ids (str): id of question
        Returns: list of questions as levels
    """
    # levels = {}
    try:  
...
def get_magogenie_info_url():
    """    
    get_magogenie_info_url: Generates tree from the content downloaded from magogenie in json format
    """
    
    TREE_URL = "https://www.magogenie.com/v1/tree/39e84462-60f6-49a6-a540-b885e7802d07"
    SAMPLE = []
    data = {}
    try:
    ...
    for key in ['BalBharati', 'CBSE']:#sorted(data['boards'].keys())[::-1]:     
                    ...
                    if value3['question_ids']:
                        ...
                        for i in arrlevels:
                            ...
                            if i["difficulty_level"] not in levels:
                                ...
                                levels[diff] = {'id': source_id_unique, 'title': val, 'questions': [], 'mastery_model': exercises.M_OF_N, 'license': licenses.ALL_RIGHTS_RESERVED, 'domain_ns': 'GreyKite Technologies Pvt. Ltd.', 'Copyright Holder':'GreyKite Technologies Pvt. Ltd.'}
...
def build_magogenie_tree(topics):
    ...
    """
    build_magogenie_tree: Sorts topics data id wise
    """
...
def add_files(node, file_list):
    """
    add_files: Finds type of filesand adds the data to the nodes.
    """
    for f in file_list:
...
def create_question(raw_question):
    """
    create_question: Adds questions data to the exercise on the basis of type of question    
    """
...
def convert_question_content(content, q_id, flag):
    """
    convert_question_content: Processes question content and converts it into format as supported by kolibri 
    """
    #########
    ...
    while url1 in content:
        content = content.replace(url1,'').replace('../..','').replace("..",'')
        content = re.sub(REGEX_IMAGE, lambda m:url2+"{}".format(m.group(0)) if url1 not in m.group(0) else "{}".format(m.group(0)), content)
    
    # else:
    content = content.replace(url2,'').replace('../..','').replace("..",'')
    content = re.sub(REGEX_IMAGE, lambda m: url2+"{}".format(m.group(0)) if url2 not in m.group(0) else "{}".format(m.group(0)), content)
    # content = content.replace('../..','').replace("..",'')
    # content = re.sub(REGEX_IMAGE, lambda m: "{}".format(m.group(0)), content)
    ...
    # content = re.sub(MATHML_REGEX, lambda m: "image/png".format(m.group(0)), content)

    # if "<mspace linebreak='newline'>" in content:
    #   content = content.replace("<mspace linebreak='newline'>","<mspace linebreak='newline'> /")
    ...
    content = content.replace("\_\_\_{_}","\\_\\_\\_\\_")
    #else:
...
def mathml_to_latex(match, q_id):
    """
    mathml_to_latex: Converts mathml in content to latex, so that it can be rendered in kolibri
    """
    regex = r"\\overline{\)(.*?)}"
    ...
    #print ("match:", match)
    if "<mspace linebreak='newline'>" in match:
        match = match.replace("<mspace linebreak='newline'>","<mspace linebreak='newline'/>")

    if "&quot" in match:
        match = match.replace("&quot;","\"")


    path = "/home/kolibri/Divya/Magogenie/magogenie-channel/q_files"
    ...
    try:
        ...
        if len(re.findall(r"{\\right\)}", text)) > 0:
...
# # print(type(folder_dict))
# folder_lst=[]
# # print(type(folder_lst))
# folder_lst.append(folder_dict)
# # print(type(folder_lst))
...
class Upload(SushiChef):
    ...
    channel_info = {    #
        'CHANNEL_SOURCE_DOMAIN' : "magogenie.com",
        # 'CHANNEL_SOURCE_ID' : "Magogenie BalBharati Final Import",
        # 'CHANNEL_TITLE' : "Magogenie BalBharati",
        'CHANNEL_SOURCE_ID' : "Magogenie Master Channel -2019",
        'CHANNEL_TITLE' : "Magogenie Master Channel",
        'CHANNEL_LANGUAGE' : 'en'
